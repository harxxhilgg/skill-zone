  ...
  return (
    <div >
      <Table className='rounded-xl border border-richblack-800'>
        ...
        <Tbody>
          {courses?.length === 0 ? (
            <Tr><Td className="py-10 text-center text-2xl font-medium text-richblack-100">No Courses found</Td></Tr>
          ) : (courses?.map((course) => (
            <Tr key={course._id}
            className="flex gap-x-10 border-b border-richblack-800 px-6 py-8">
              <Td className="flex flex-1 gap-x-4">
                ...
                <div className='flex flex-col justify-between'>
                  ...
                  {course.status === COURSE_STATUS.DRAFT ? (
                    <p className='flex w-fit flex-row items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-pink-100'><HiClock size={14}/> Drafted</p>
                  ): (
                    <p className='flex w-fit flex-row items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-yellow-100'><div>
                      <FaCheck size={8}/>
                    </div>Published</p>
                  )}
                </div>
              ...
              title='Edit'
              className='px-2 transition-all duration-200 hover:scale-100 hover:text-caribbeangreen-300'>
                <FiEdit2 size={20}/>
              </button>
              <button
              disabled={loading}
                    ...
                    title="Delete"
                    className='px-1 transition-all duration-200 hover:scale-100 hover:text-[#ff0000]'>
