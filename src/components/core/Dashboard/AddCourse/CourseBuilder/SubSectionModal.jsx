...
const SubSectionModal = ({modalData,setModalData,add=false,view=false,edit=false}) => {
  ...
  const isFormUpdated = () => {
    ...
    if (
      currentValues.lectureTitle !== modalData.title || currentValues.lectureDesc !== modalData.description || 
    ...
    <div className='fixed inset-0 z-[1000] !mt-0 grid place-items-center h-screen w-screen overflow-auto bg-white bg-opacity-10 backdrop-blur-sm'>
      <div className='my-10 w-11/12 max-w-[700px] rounded-lg border border-richblack-400 bg-richblack-800'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-8 px-8 py-10'>
          ...
          <div className='flex flex-col space-y-2'>
            <label htmlFor='lectureDesc' className='text-sm text-richblack-5'>Lecture Description {" "} {!view && <sup className='text-pink-200'>*</sup>}</label>
            <textarea
              disabled={view || loading}
              id='lectureDesc'
