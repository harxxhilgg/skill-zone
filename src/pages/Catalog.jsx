...
//       const category_id = 
//             res?.data?.data?.find((ct) => ct.name.split(" ").join("-").toLowerCase() === catalogName)[0]._id;
...
//   },[catalogName]);
  ...
  useEffect(() => {
    const getCategories = async () => {
      try {
        const res = await apiConnector("GET", categories.CATEGORIES_API);
        const filteredCategories = res?.data?.data || []; // Ensure res.data.data is an array
        const matchedCategory = filteredCategories.find((ct) =>
        ...
        if (matchedCategory) {
          ...
          console.log(`Category '${catalogName}' not found.`);
  ...
  return (
    ...
    <div className='max-w-maxContentTab flex-col justify-center gap-4 mx-auto flex min-h-[260px] lg:max-w-maxContent'>
